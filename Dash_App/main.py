    ...
    dbc.Row(
        dbc.Col([
            dcc.Dropdown(id='features', options=[{'label': 'hour_of_day', 'value': 'hour_of_day'},
                                                 ...
                                                 {'label': 'top_10_words', 'value': 'top_10_words','disabled':True}], multi=True,value=['price'],placeholder="Select Features That Will Not Be Used In Training")
        ]),
    ),
    dbc.Row([
        dbc.Col(html.Button('Train And Predict',id='predict',n_clicks=0)),
    ]),
    dbc.Row([
        dbc.Col(
        ...
        dbc.Col(
            ...
            dbc.Col(
                html.Div(id='output2',children=[html.H1()])
            )]),
        dbc.Row(
            dbc.Col(
        ...
        dbc.Row([
            ...
            dbc.Col(
                dcc.Graph(figure=px.bar(x=outputFive.index[:-1],y=outputFive.values[:-1],title="Most Words Used"))
            ),
        ]),
    dbc.Row(
            html.Img(id='example')
...
def update_features(predicts,featuress):
    ...
    if predicts:
        ...
        outputFour=px.bar(data_frame=outputFour,x=outputFour.index,y=outputFour.values,title="Model Features Importance")
