...
df = pd.read_csv ('ScrappedReddit.csv')
...
app.layout=dbc.Container([
    ...
    dbc.Row(html.H1('Assests')),
    dbc.Row(html.Button('Predict',id='predict',n_clicks=0)),
    dbc.Row([
        dbc.Col(
        ...
        dbc.Col([
                DataTable(id='predictions')
            ],className='pt-2')
        ]),
    dbc.Row(html.H1('Features To Drop')),
    dbc.Row([
        dbc.Col(
            dcc.Dropdown(id='features',options=[{'label':'hour_of_day','value':'hour_of_day'},
                                                  ...
                                                  {'label':'top_10_words','value':'top_10_words'}],multi=True,clearable=True)
            ),
...
lists=['comments','pctChange','price','volume']





@app.callback(Output('output1','data'),Output('output1','columns'),Output('output0','data'),Output('output0','columns'),Output('output2','children'),Output('output3','children'),Output('output4','figure'),Output('example', 'src'),Input('predict','n_clicks'),State('features','value'))
def update_features(predicts,featuress):
    ...
    if predicts:
        ...
        outputThree = "Prediction of 8: "+str(rf_model.predict(newRowToPredict.values.reshape(8, -1)))
        ...
        featuresTest['Predictions'] = testResults
        featuresTest['Predictions'] = featuresTest['Predictions'].map({0: 'Sell', 1: 'Buy'})
        featuresTest.to_csv('prediction.csv')
        return df_report,reports_columns,df_reporttranspose,reporttranspose_columns,outputTwo,outputThree,outputFour,"data:image/png;base64,{}".format(data)





@app.callback(Output('predictions','data'),Output('predictions','columns'),Input('assets','value'))
def update_predictions(assets):
    print(assets)
    dff_prediction =pd.read_csv('prediction.csv')
    print(dff_prediction.head())
    print(dff_prediction.columns)
    dff_prediction=dff_prediction.loc[dff_prediction[assets]==1]['Predictions']
    dff_prediction=pd.DataFrame({'Predictions':dff_prediction})
    print(dff_prediction)
    columns = [{"name": i, "id": i} for i in dff_prediction.columns]
    print(columns)
    dff_prediction = dff_prediction.to_dict('records')
    return dff_prediction,columns
